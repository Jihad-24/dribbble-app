...
const CreateProfile = ({ singleUserData }) => {
  ...
  const [selectFirstBox, setSelectFirstBox] = useState(
    singleUserData?.whyICame?.selectFirstBox || false
  );
  const [selectSecondBox, setSelectSecondBox] = useState(
    singleUserData?.whyICame?.selectSecondBox || false
  );
  const [selectThirdBox, setSelectThirdBox] = useState(
    singleUserData?.whyICame?.selectThirdBox || false
  );
  const [ProfilePhotoPlaceholder, setProfilePhotoPlaceholder] = useState(
    singleUserData?.image || addPhotoImg
  );
  ...
  const handleUpdateProfile = async () => {
    ...
    updateProfile(auth.currentUser, {
      ...
      .then(() => {
        ...
        axiosPublic
          .put(`/updateUserData/${singleUserData?._id}`, allData)
          .then((res) => {
  ...
  return (
    <div className="w-full">
      ...
      <div className={`w-full relative min-h-[95vh]`}>
        <div className="w-full h-full absolute top-0">
          <div
            ...
            } max-w-[550px] mx-auto pt-24 px-10 sm:px-0 pb-5`}
          >
            <div className="mx-auto">
              ...
              <div className="">
                <button
                  ...
                  className={`${
                    !showPreviousBtn
                      ? location && ProfilePhotoPlaceholder !== addPhotoImg
                        ? "block"
                        : "hidden"
                      : "hidden"
                  }`}
                >
                  <span className="btn bg-primary text-white text-base w-[200px] hover:bg-primary">
                    Next
                  </span>
                </button>
                <button
                ...
                >
                  <a className=" btn active:scale-100 bg-primary/50 hover:bg-primary/50 cursor-not-allowed text-white text-base w-[200px]">
                    Next
                  </a>
                </button>
              </div>
        ...
        <div className={`w-full absolute top-0`}>
          <div
            className={`transition-all duration-500 origin-right w-full ${
              !showPreviousBtn
                ? "opacity-0 scale-x-0 h-0"
                : "opacity-100 scale-x-100 flex justify-center items-center min-h-[95vh] pt-24 pb-10 px-10"
            } `}
          >
            <div className="mx-auto w-full">
              ...
              <div className="w-full mx-auto flex flex-col justify-center items-center gap-5 pt-7 ">
                ...
                <button
                  ...
                  disabled={
                    !selectFirstBox && !selectSecondBox && !selectThirdBox
                  }
                  className="btn bg-primary/90 text-white text-base w-[200px] hover:bg-primary"
                >
                  Finish
                </button>
              </div>
      ...
      <div className="">
        <a
          onClick={() => setShowPreviousBtn(false)}
          className={`${showPreviousBtn ? "block" : "hidden"}`}
        >
          <span className="fixed top-7 left-40 btn w-max px-5 text-xl">
            {" "}
            <FontAwesomeIcon icon={fas.faLessThan} />{" "}
          </span>{" "}
        </a>
      </div>
    </div>
