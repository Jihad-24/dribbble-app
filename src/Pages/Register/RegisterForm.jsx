...
const RegisterForm = () => {
    ...
    createUser(email, password)
      .then((res) => {
        ...
        updateProfile(auth.currentUser, {
          ...
          .then(() => {
            axiosPublic.post("/addUser", allData).then((res) => {
  ...
  const handleUserName = (e) => {
    ...
    if (e.target.value === "") {
      ...
      setUserName(e.target.value);
      axiosPublic
        .get(`/checkExistingUserName/${e.target.value}`)
        .then((res) => {
          if (res.data.isExist) {
    ...
    <div className="w-full max-w-[500px] space-y-5">
      <div className="relative pb-7">
        ...
        <ul className="text-primary  absolute bottom-0 left-5 text-sm font-semibold">
          {isUserNameError === 3 && (
            <li className="list-disc">Please take a username.</li>
          )}
          {isUserNameError === 1 && (
            <li className="list-disc">Username already has been taken</li>
          )}
          {isUserNameError === 0 && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="flex gap-5">
          ...
          <span className="font-semibold">
            ...
            <span className="text-[#443496]">
              Terms of Service, Privacy Policy,
            </span>{" "}
            and our default{" "}
            <span className="text-[#443496]">Notification Settings.</span>
          </span>
        </div>
        <div className="flex gap-5">
          {registering ? (
            <button className="btn bg-primary/90 text-white text-base w-[200px] hover:bg-primary">
            ...
            <button
              disabled={!isChecked}
              className="btn bg-primary text-white text-base w-[200px] hover:bg-primary"
            >
              Create Account
            </button>
          )}
